...
<body class="antialiased bg-gray-200 text-gray-900 font-sans p-6">
    <div class="container mx-auto">
      <div class="flex flex-wrap -mx-4">
        @if($paidHouses->isNotEmpty())
        @foreach($paidHouses as $house)
        <div style="" class="w-full sm:w-1/2 md:w-1/2 xl:w-1/4 p-4 shadow-xl">
          <a href="{{ route('renter.houses.details', $house->house_id) }}" class="c-card block bg-white shadow-md hover:shadow-xl rounded-lg overflow-hidden">
          ...
          <div class="p-4">
            @if($house->payment_status === "COMPLETED")
            <center><h4 style="color: black" class="mt-2 mb-2  font-bold">Name: {{$house->user->name}}</h4></center>
            @else
            <center><h4 style="color: black" class="mt-2 mb-2  font-bold">Name: Pay To View</h4></center>
            @endif
            @if($house->payment_status === "COMPLETED")
            <center> <a href="tel:{{$house->user->contact}}"> <h4 id="number" style="color: black;" class="mt-2 mb-2  font-bold"><i style="color: gray; margin-right: 5px" class="fa fa-phone"></i> {{$house->user->contact}}</a><button style="padding-left: 10px; font-size:12px" onclick="copyNumber('number')">Copy Number</button></h4></center>
            @else
          ...
          <div class="p-4 border-t border-b text-xs text-gray-700">
            ...
            <span class="flex items-center mb-1">
                ...
                @else
                <p style="font-size: 16px; white-space: nowrap; color:red">Status : PENDING</p>
                @endif
                </div>
                <!-- /.col-sm-3 -->
        ...
        @endforeach
        @else
        <h2 class="center">No Paid Houses</h2>
        @endif
      </div>
    </div>
  </body>
