            ...
            <div class="col-md-12">
               @include('partial.successMessage')
                ...
                <div class="card my-5 mx-4">
                    ...
                    <a href="{{route('dalali.house.create')}}" class="btn btn-success btn-md float-right c-white">Add new <i class="fa fa-plus"></i></a>
                    </div>
                    <!-- /.card-header -->
                    @if ($houses->count() > 0)
                    <div class="">
                      ...
                      <table id="dataTableId" class="table table-bordered table-striped table-background">
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <a href="{{ route('dalali.house.status', $house->id) }}"  class="btn btn-warning btn-sm ">Switch Status</a>
                            <a href="{{ route('dalali.house.show', $house->id) }}"  class="btn btn-success btn-sm">Details</a>
                            <a href="{{ route('dalali.house.edit', $house->id) }}"  class="btn btn-info btn-sm">Edit</a>
                            <button class="btn btn-danger btn-sm" type="button" onclick="deleteHouse({{ $house->id }})">
                                Delete
                            </button>

                          <form id="delete-form-{{ $house->id }}" action="{{ route('dalali.house.destroy',$house->id) }}" method="POST" style="display: none;">
                              @csrf
                              @method('DELETE')
